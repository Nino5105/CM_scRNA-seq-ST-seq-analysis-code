...
gene <- c(
"Ccl3","Ccl4","Ifnb1","Il13","Csf1","Csf2",
"Ifng",
"Ccl5",
"H2-Aa","H2-Ab1",
"H2-K1","H2-D1",
...
MVC <- FindMarkers(Micro_Inflam,ident.1 = "Model",ident.2 = "Control",
...
ggplot(MVC, aes(x= -log10(p_val_adj),y=avg_log2FC))+geom_point(aes(color=type))+geom_hline(yintercept=c(-0.25,0.25))+ggtitle("Micro_Inflam_DEGs\n(ModvsCon)")


AVC <- FindMarkers(Micro_Inflam,ident.1 = "ART",ident.2 = "Control",
                              logfc.threshold = 0,only.pos = F,min.diff.pct = 0)
...
ggplot(AVC, aes(x= -log10(p_val_adj),y=avg_log2FC))+geom_point(aes(color=type))+geom_hline(yintercept=c(-0.25,0.25))+ggtitle("Micro_Inflam_DEGs\n(ARTvsCon)")
